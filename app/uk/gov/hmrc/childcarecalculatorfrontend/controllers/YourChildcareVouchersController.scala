...
class YourChildcareVouchersController @Inject() (
    ...
    yourChildcareVouchers: yourChildcareVouchers
  ...
  def onPageLoad(): Action[AnyContent] = getData.andThen(requireData) { implicit request =>
    ...
    Ok(yourChildcareVouchers(preparedForm))
  }

  def onSubmit(): Action[AnyContent] = getData.andThen(requireData).async { implicit request =>
    BooleanForm(yourChildcareVoucherErrorKey)
      .bindFromRequest()
      .fold(
        (formWithErrors: Form[Boolean]) => Future.successful(BadRequest(yourChildcareVouchers(formWithErrors))),
        value =>
          dataCacheService
            .save(YourChildcareVouchersId, value)
            .map(cacheMap => Redirect(navigator.nextPage(YourChildcareVouchersId)(new UserAnswers(cacheMap))))
      )
  }
