    ...
    getData.andThen(requireData).async { implicit request =>
      request.userAnswers.aboutYourChild(childIndex) match {
        case Some(child) =>
          ...
          Future.successful(Ok(childcarePayFrequency(preparedForm, childIndex, child.name)))
      ...
      request.userAnswers.aboutYourChild(childIndex) match {
        case Some(child) =>
          ChildcarePayFrequencyForm(child.name)
            .bindFromRequest()
            .fold(
              (formWithErrors: Form[ChildcarePayFrequency]) =>
                Future.successful(
                  BadRequest(childcarePayFrequency(formWithErrors, childIndex, child.name))
                ),
              value =>
                dataCacheService
                  .saveInMap[Int, ChildcarePayFrequency](
