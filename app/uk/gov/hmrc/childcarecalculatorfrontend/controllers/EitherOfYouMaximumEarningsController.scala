...
class EitherOfYouMaximumEarningsController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = getData.andThen(requireData) { implicit request =>
    ...
    Ok(eitherOfYouMaximumEarnings(preparedForm))
  }
  ...
  def onSubmit(): Action[AnyContent] = getData.andThen(requireData).async { implicit request =>
    BooleanForm(eitherOfYouMaximumEarningsErrorKey)
      .bindFromRequest()
      .fold(
        (formWithErrors: Form[Boolean]) => Future.successful(BadRequest(eitherOfYouMaximumEarnings(formWithErrors))),
        value =>
          dataCacheService
            .save(EitherOfYouMaximumEarningsId, value)
            .map(cacheMap => Redirect(navigator.nextPage(EitherOfYouMaximumEarningsId)(new UserAnswers(cacheMap))))
      )
  }
