...
class YourMaximumEarningsController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = getData.andThen(requireData) { implicit request =>
    ...
    Ok(yourMaximumEarnings(preparedForm))
  }
  ...
  def onSubmit(): Action[AnyContent] = getData.andThen(requireData).async { implicit request =>
    BooleanForm(yourMaximumEarningsErrorKey)
      .bindFromRequest()
      .fold(
        (formWithErrors: Form[Boolean]) => Future.successful(BadRequest(yourMaximumEarnings(formWithErrors))),
        value =>
          dataCacheService
            .save(YourMaximumEarningsId, value)
