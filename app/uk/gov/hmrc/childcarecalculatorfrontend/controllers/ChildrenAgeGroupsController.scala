...
class ChildrenAgeGroupsController @Inject() (
  ...
  def onSubmit(): Action[AnyContent] = getData.andThen(requireData).async { implicit request =>
    ChildrenAgeGroupsForm()
      .bindFromRequest()
      .fold(
        formWithErrors => Future.successful(BadRequest(childAgeGroup(formWithErrors))),
        value =>
          dataCacheService
            .save(ChildrenAgeGroupsId, value)
            .map(cacheMap => Redirect(navigator.nextPage(ChildrenAgeGroupsId)(new UserAnswers(cacheMap))))
      )
