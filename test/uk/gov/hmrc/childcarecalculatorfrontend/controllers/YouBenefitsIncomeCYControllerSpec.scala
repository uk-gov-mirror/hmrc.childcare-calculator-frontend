...
class YouBenefitsIncomeCYControllerSpec extends ControllerSpecBase {
  ...
  "YouBenefitsIncomeCY Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData       = Map(YouBenefitsIncomeCYId.of(testNumber))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad()(fakeRequest)

      contentAsString(result) mustBe viewAsString(YouBenefitsIncomeCYForm().fill(testNumber))
    }
