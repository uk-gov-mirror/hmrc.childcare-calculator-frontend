...
class YourMaximumEarningsControllerSpec extends ControllerSpecBase {

  val view: yourMaximumEarnings = inject[yourMaximumEarnings]

  def onwardRoute: Call = routes.WhatToTellTheCalculatorController.onPageLoad

  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new YourMaximumEarningsController(
      mcc,
  ...
  "YourMaximumEarnings Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData       = Map(YourMaximumEarningsId.of(true))
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val boundForm   = BooleanForm(yourMaximumEarningsErrorKey).bind(Map("value" -> "invalid value"))
