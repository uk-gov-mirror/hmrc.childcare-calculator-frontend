...
class PartnerChildcareVouchersControllerSpec extends ControllerSpecBase {
  ...
  val view: partnerChildcareVouchers = inject[partnerChildcareVouchers]
  def onwardRoute: Call              = routes.WhatToTellTheCalculatorController.onPageLoad

  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new PartnerChildcareVouchersController(
      mcc,
  ...
  "PartnerChildcareVouchers Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData       = Map(PartnerChildcareVouchersId.of(true))
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val boundForm   = BooleanForm("partnerChildcareVouchers.error.notCompleted").bind(Map("value" -> "invalid value"))
